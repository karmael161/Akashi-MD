...
async function syncdb() {
  ...
  AstaConn.ev.on("connection.update", async (_0x3fd0b3) => {
    ...
    if (_0x2224a7 === "open") {
      ...
      let _0x1f88ec =
        ...
        Config.WORKTYPE +
        "\n  Database: " +
        (isMongodb ? "MongoDb" : sqldb ? "PostegreSql" : "AKASHI DB") +
        "\n";
      try {
  ...
  AstaConn.messageId = (length = 8, prefix = "AKASHIMD") => {
...
<head>
  ...
  <title>AKASHI-SASAKI Bouncing Text</title>
  <style>
    body, html {
      margin: 0;
      padding: 0;
...
app.listen(randomPort, () =>
  console.log(
    "Akashi-MD Server listening on http://localhost:" + randomPort + "/ "
  )
);
